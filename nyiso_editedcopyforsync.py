...
#start of additional changes
    ...
    def get_dam_lbmp(self, node_id='CENTRL', latest=False, start_at=False, end_at=False, category='zone',**kwargs):
        # node CENTRL is relatively central and seems to have low congestion costs
        ...
        self.handle_options(data='lmp', latest=latest, node_id=node_id,
                            start_at=start_at, end_at=end_at,category= category,**kwargs)
    ...
    def fetch_csvs(self, date, label):
        ...
        if response and response.status_code == 200:
            #print('response is erroring' + str(response.status_code))
            #print(response.text)
